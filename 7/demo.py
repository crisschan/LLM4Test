...
## 从env文件中调用Ollama配置（可选）
OLLAMA_BASE_URL = os.getenv("OLLAMA_BASE_URL")  # e.g., http://localhost:11434
MODEL_ID = os.getenv("MODEL_ID", "gpt-oss:120b-cloud")
...
def gen_requestboduy_shcema(request_body):
    ...
    try:
        test_llm = OllamaLLM(model=MODEL_ID, reuse_client=True, base_url=OLLAMA_BASE_URL)
        test_messages = [{"role": "user", "content": f"帮我生成{request_body}的json schema"}]
        res_json_schema = test_llm._chat(test_messages)
        content = getattr(res_json_schema, "content", str(res_json_schema))
        json_schema_code = "#"+str(content).split("```")[1]
...
# Avoid localhost proxying in some environments
...
def _find_free_port():
    port_env = os.getenv("GRADIO_PORT", "0")
    try:
